...
const App = () => {
  ...
  const handlePostDrop = async (e) => {
    ...
    if (isUploading) {
      setErrorMsg("Please wait for photo upload to hit 100%");
      return;
    }
    if (newDrop.images.length === 0) {
      setErrorMsg("Take a photo before posting!");
      return;
    }
    ...
    navigator.geolocation.getCurrentPosition(async (pos) => {
      try {
        ...
        setErrorMsg(`SAVE FAILED: ${err.code === 'permission-denied' ? 'Firestore Rules are locked.' : err.message}`);
      } finally { setIsPosting(false); }
    }, () => {
      setErrorMsg("GPS Required. Enable Location in Phone Settings.");
      setIsPosting(false);
    });
  };
  ...
  const MapView = () => {
    ...
    useEffect(() => {
      ...
      drops.filter(d => d.title?.toLowerCase().includes(searchTerm.toLowerCase())).forEach(d => {
        if (!d.lat) return;
  ...
  if (!firebaseConfig.apiKey) return (
    <div className="h-screen flex items-center justify-center p-10 text-center bg-slate-900 text-white font-sans">
      <div>
        ...
        <h2 className="text-2xl font-black uppercase italic tracking-tighter">Missing API Keys</h2>
        <p className="text-slate-400 text-sm mt-3">Check Vercel Environment Variables.</p>
      </div>
    </div>
  );
          ...
          <>
            ...
            {displayMode === 'list' ? (
              <div className="px-4 space-y-4 pb-32">
                {drops.length === 0 && <div className="py-20 text-center text-slate-300 italic text-sm">No live ants found...</div>}
                    ...
                    <div className="p-5 flex justify-between items-center">
                      <div>
                        <div className="flex gap-1 mb-1">
                          ...
                          {d.hasCoupon && <span className="bg-pink-100 text-pink-600 text-[8px] font-black px-2 py-0.5 rounded-md uppercase tracking-tighter font-black">10% OFF</span>}
                        </div>
          ...
          <div className="pb-40 bg-white min-h-screen animate-in slide-in-from-right">
            ...
            <div className="p-8 -mt-10 bg-white rounded-t-[48px] relative z-10 space-y-6 shadow-2xl">
              <div className="flex justify-between items-center">
                 ...
                 <div className="bg-green-100 text-green-600 px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest">Live Now</div>
              </div>
              
              <div className="flex gap-3">
                <button onClick={() => window.open(`https://maps.google.com/?q=${selectedDrop.lat},${selectedDrop.lng}`)} className="flex-1 bg-slate-100 p-4 rounded-3xl flex flex-col items-center gap-1 active:bg-slate-200 transition-colors"><Navigation className="w-6 h-6 text-slate-600"/><span className="text-[10px] font-black uppercase text-slate-500 tracking-widest">Maps</span></button>
              ...
              <div className="space-y-2">
                <h3 className="font-black text-[10px] uppercase text-slate-400 tracking-widest flex items-center gap-2 px-1 italic"><ShoppingBag className="w-3 h-3" /> Merchant Menu</h3>
              ...
              <div className="bg-slate-900 p-6 rounded-[32px] flex justify-between items-center text-white active:bg-black transition-colors shadow-xl shadow-indigo-100" onClick={()=>setShowPayment(true)}>
                <div className="text-left"><p className="text-[10px] font-bold opacity-70 uppercase tracking-widest mb-1 italic">Scan to Pay Merchant</p><p className="font-bold text-lg tracking-tight underline decoration-indigo-400 underline-offset-4">{selectedDrop.zelleId}</p></div>
        ...
        {view === 'post' && (
          <div className="p-8 space-y-6 pb-40 animate-in slide-in-from-bottom font-sans">
            ...
            <div className="space-y-4">
              ...
              >
                {isPosting ? 'Publishing...' : 'Go Live on Map'}
              </button>
            </div>
          </div>
        )}

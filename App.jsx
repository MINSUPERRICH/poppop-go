...
const App = () => {
  ...
  const handleFileChange = async (e) => {
    ...
    if (!fbStorage) {
      setErrorMsg("Storage not initialized. Check your Bucket URL in Vercel.");
      return;
    }
      ...
      for (let i = 0; i < files.length; i++) {
        ...
        try {
          ...
          setErrorMsg(uploadErr.code === 'storage/unauthorized' 
            ? "PERMISSION DENIED: You must Publish Storage Rules in Firebase." 
    ...
    <div className="h-screen flex items-center justify-center p-10 text-center bg-slate-950 text-white font-sans">
      <div className="space-y-6">
        ...
        <p className="text-slate-400 text-xs leading-relaxed max-w-xs mx-auto italic font-medium">
          Check Vercel Dashboard {"->"} Settings {"->"} Env Variables. Then click Redeploy.
        </p>
      </div>
    </div>
  );
        ...
        {view === 'post' && (
          <div className="p-8 space-y-6 pb-40 animate-in slide-in-from-bottom font-sans">
            ...
            <div className="space-y-4">
              ...
              <button onClick={handlePostDrop} className="w-full bg-indigo-600 text-white py-5 rounded-[28px] font-black shadow-xl uppercase tracking-widest text-xs active:scale-95">Go Live on Map</button>
            </div>
          </div>
        )}
      </main>
